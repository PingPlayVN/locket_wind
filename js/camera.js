...
async function loadFriendsList(dom) {
    if (window.isFriendsLoaded) return; // Đã tải rồi thì thôi
        ...
        friendUids.forEach(async (uid) => {
            try {
                ...
                if (profile) {
                    ...
                    let friendName = `${lastName} ${firstName}`.trim();
                    if (!friendName) friendName = username ? `@${username}` : "Bạn bè Locket";
                    ...
                    if (itemEl) {
                        ...
                        itemEl.querySelector('.send-name').innerText = friendName;
